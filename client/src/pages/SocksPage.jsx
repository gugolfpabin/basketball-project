...
export default function SocksPage() { // Changed component name
  const navigate = useNavigate();
  const [anchorElCategory, setAnchorElCategory] = useState(null);
  const [anchorElUser, setAnchorElUser] = useState(null);
  const [user, setUser] = useState(null);
  const [products, setProducts] = useState([]); // State to store socks products
  ...
  useEffect(() => {
    ...
    // Function to fetch Socks from Backend API
    const fetchSocks = async () => { // Changed function name
      try {
        // IMPORTANT: Ensure 'socks' is mapped to a Category_ID in your backend's productController.js
        const response = await axios.get(`${apiBase}/products?category=socks`); // Changed category
        setProducts(response.data);
        console.log("Fetched Socks products:", response.data);
      } catch (error) {
        console.error("Error fetching Socks products:", error);
        setProducts([]);
      }
    };

    fetchSocks(); // Call the function to fetch socks
  }, []);
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      {/* Main content for Socks */}
      <Container sx={{ py: 4 }}>
        <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 4 }}>
          ถุงเท้า {/* Changed Header Text */}
        </Typography>
