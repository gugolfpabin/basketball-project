...
export default function AddProductPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        colorVariants.forEach(cv => {
            if (cv.color && cv.sizes.length > 0 && cv.price !== '' && cv.cost !== '') {
        ...
        if (!hasValidVariant) {
            setSnackbarMessage('กรุณากรอกรายละเอียดสี, ขนาด, และสต็อกอย่างน้อยหนึ่งรายการ');
            setSnackbarSeverity('error');
            setSnackbarOpen(true);
            setLoading(false);
            return;
        ...
        try {
            ...
            const productData = {
                ...
                variants: variantsToCreate.map(v => ({
                    ...v,
                    imageUrl: colorImageUrlMap[v.color],
                })),
            };
            ...
            setCategoryId('');
            setColorVariants([{ color: '', sizes: [], price: '', cost: '', imageFile: null, imageUrl: '' }]);
            ...
            setSnackbarMessage(`เกิดข้อผิดพลาดในการเพิ่มสินค้า: ${err.response?.data?.message || err.message}`);
            setSnackbarSeverity('error');
            setSnackbarOpen(true);
        } finally {
            setLoading(false);
